...
logger = logging.getLogger('huajun')
...
class HuaJunSpider(object):
    ...
    def redisHset(self, key, item, name='huajun'):
        """
        redis持久化
        """
    ...
    def parse(self):
        ...
        for html in self.start_requests():
            ...
            for url in ehtml.xpath('//div[@class="g-map"]/div[2]/div[2]/div/a/@href')[:2]:
    ...
    def page_url(self):
        ...
        for url in self.parse():
            ...
            ehtml = etree.HTML(html)
            ids = ehtml.xpath('//div[@class="g-pages"]/a[last()-1]/text()')
            ids = ids[0]
            if ids:
                ids = int(ids)
            else:
    ...
    def list_url(self):
        ...
        for url in self.page_url():
            html = self.getRequest(url)
            ehtml = etree.HTML(html)
            for url in ehtml.xpath('/html/body/section/div/div[1]/div[3]/ul/li/a/@href'):
    ...
    def shop_html(self):
        ...
        for url in self.list_url():
            ...
            item['downloadUrl'] = ehtml.xpath('//*[@id="downBox"]/div/div[1]/p/a[1]/@href')
            item['shop'] = '华军软件园'
            item['url'] = url
